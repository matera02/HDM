...
public class NSP {
    ...
    protected int numShifts;
    protected int[][] hospitalCoverage;
    protected int[][][] nursePreferences;
    ...
    protected int[][][] randomSchedule() {
        int[][][] schedule = new int[this.numNurses][this.numDays][this.numShifts];
        for(int i = 0; i < this.numNurses; i++){
            for(int k = 0; k < this.numDays; k++){
                int s = this.random.nextInt(this.numShifts);
                schedule[i][k][s] = 1;
            }
        }
        return schedule;
    ...
    protected boolean isFeasible(int[][][] schedule) {
        ...
        for(int k = 0; k < this.numDays; k++){
            for(int s = 0; s < this.numShifts; s++){
                int sum = 0;
                for(int i = 0; i < this.numNurses; i++){
                    sum += schedule[i][k][s];
                }
                if(sum < this.hospitalCoverage[k][s]){
                    return false;
                }
            }
        }
        ...
        for(int i = 0; i < this.numNurses; i++){
            for(int k = 0; k < this.numDays - 1; k++){
    ...
    protected double fitness(int[][][] schedule) {
        ...
        for(int i = 0; i < this.numNurses; i++){
            for(int k = 0; k < this.numDays; k++){
                for(int s = 0; s < this.numShifts; s++){
                    nurseAversion += schedule[i][k][s] * this.nursePreferences[i][k][s];
                }
            }
        }
        ...
        for(int k = 0; k < this.numDays; k++){
            for(int s = 0; s < this.numShifts; s++){
                ...
                int diff = coverage - this.hospitalCoverage[k][s];
                hospitalCost += diff;
            }
        }
